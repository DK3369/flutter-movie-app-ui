...
class LyricPainter extends CustomPainter with ChangeNotifier {
  ...
  //字体最大宽度
  double? lyricMaxWidth = 0.0;

  //歌词间距
  double? lyricGapValue = 0.0;

  //歌词间距
  double? subLyricGapValue = 10.0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    lyricTextPaints[currentLyricIndex]
      ...
      ..layout(maxWidth: lyricMaxWidth ?? 10.0);
    ...
    for (int lyricIndex = 0; lyricIndex < lyrics!.length; lyricIndex++) {
      ...
      currentLyricTextPaint.layout(maxWidth: lyricMaxWidth ?? 10.0);
      ...
      if (subLyrics != null) {
        ...
        remarkLyrics.forEach((remarkLyric) {
          ...
          currentSubPaint..layout(maxWidth: lyricMaxWidth ?? 10.0);
          //当前歌词结束后调整下次开始绘制歌词的y坐标
          currentLyricY += currentSubPaint.height + (subLyricGapValue ?? 10.0) ;
        });
      }
    }
  }
